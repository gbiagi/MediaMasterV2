...
@Composable
fun GameView(id: String, goTo: (Screen) -> Unit) {
    ...
    LaunchedEffect(id) {
        try {
            displayedGame = ApiClient.getGame(id)
            isLoading = false
        } catch (e: Exception) {
        ...
        when {
            ...
            displayedGame != null -> {
                Column(
                    ...
                    KamelImage(
                        resource = asyncPainterResource(displayedGame!!.imageUrl),
                    ...
                    Text(text = "Description: ${displayedGame!!.description}")
                    Spacer(Modifier.height(8.dp))
                    Text(text = "Release date: ${displayedGame!!.releaseYear}")
                }
            }
        }
    }
